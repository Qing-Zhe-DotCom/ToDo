...
本文档描述的是当前仓库的真实架构状态；未来已确定路线会单独列出，但不会把目标态误写成当前已落地事实。

如果你是第一次接手仓库，建议先读 [README.md](./README.md)，再读本文档。  
如果你更关心数据库和运行基线，请看 [DATABASE_AND_RUNTIME.md](./DATABASE_AND_RUNTIME.md)。

## 1. 当前架构一句话概括

当前项目已经从“UI 直连 DAO 的单体 JavaFX 工程”演进到“FXML shell + ApplicationContext 组合根 + config/data/application 初步分层”的过渡态架构，但还没有完成到最终目标态。
...
- 负责 FXML shell controller 和完成状态协同逻辑
- 当前核心对象：
  - `MainController`
...
当前已经引入的 FXML 不是“全量页面都改成 FXML”后的最终形态，而是一个较轻量的外壳层。

当前真实情况：

- FXML 入口只有 `main-shell.fxml`
- `MainController` 负责装配左侧边栏、中央视图切换和右侧详情面板
- 主要业务视图仍由 Java 代码构造，而不是每个页面都有独立 FXML / Controller / ViewModel

这意味着：

- 当前已经从纯代码启动根节点走向 `FXMLLoader`
- 但还没有完成到“每个屏幕都有独立 FXML + Controller + ViewModel”的彻底重构
...
- 主题与偏好从 `MainController` 里抽出到 `ThemeService` / `JavaPreferencesStore`
...
- `ScheduleDAO` 仍存在，并且仍是数据访问链路的一部分
- `Connectdatabase` 仍代表当前默认数据库连接入口
- 默认数据库仍是 MySQL
- `Schedule` 仍是旧模型与新分层之间的共享对象
- 主要视图仍在 `com.example.view`，还没有完全迁入独立的 `ui/<screen>` 结构
...
- 视图层还没有彻底分拆为每个屏幕独立 controller/viewmodel
- 数据层还没从 MySQL 切到本地优先 SQLite
- 领域模型还没升级到分钟级时间和同步模型
- legacy DAO 还在默认链路中

所以当前最准确的描述不是“架构升级已经完成”，而是：

> 架构升级已经起步，并建立了可靠的中间态基线。
